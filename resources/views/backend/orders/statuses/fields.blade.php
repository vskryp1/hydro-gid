            ...
            <div class="row">
                <div class="col-md-6 col-md-offset-3">
                    <div>
                        ...
                        <div class="checkbox">
                            {!! Form::checkbox('processed', 1, null, ['class' => 'js-switch']) !!}
                            @lang('backend.processed')
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    @foreach(Setting::get('locales') as $lang => $locale)
                        <div class="tab-pane fade @if ($loop->first)in active @endif"
                            ...
                            <div class="form-group">
                                ...
                                {!! Form::text( $lang . '[name]', (isset($order_status)) ? $order_status->{'name:'.$lang} : null,['class'=>'form-control','placeholder'=>__('backend.name').' '.$lang,'data-validation'=>'required', 'required']) !!}
                            </div>
                        </div>
                    @endforeach
                </div>
...
@include('backend.elements.save_buttons', ['back_link' => route('backend.orders.statuses.index')])
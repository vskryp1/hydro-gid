                    ...
                    @foreach(Setting::get('locales') as $lang => $locale)
                        <div class="tab-pane fade @if ($loop->first)in active @endif"
                            ...
                            <div class="form-group">
                                ...
                                {!! Form::text( $lang . '[name]', (isset($delivery_template)) ? $delivery_template->{'name:'.$lang} : null,['class'=>'form-control','placeholder'=>__('backend.name').' '.$lang,'data-validation'=>'required', 'required']) !!}
                            </div>
                            <div class="form-group">
                                <label>@lang('backend.template')</label>
                                {!! Form::textarea( $lang . '[template]', (isset($delivery_template)) ? $delivery_template->{'template:'.$lang} : null,['class'=>'form-control ck-editor','placeholder'=>__('backend.template').' '.$lang, 'data-validation'=>'required', 'required']) !!}
                            </div>
                        </div>
                    @endforeach
                </div>
...
@include('backend.elements.save_buttons', ['back_link' => route('backend.products.delivery_templates.index')])
                ...
                <div class="col-md-6 col-md-offset-3">
                    <div>
                        ...
                        <div class="form-group">
                            ...
                            {!! Form::number('price', old('price')??(isset($customization)?round($customization->price, 2):0),['class'=>'form-control','placeholder'=> __('backend.price'),'required', 'step'=>0.01,'min'=>0.1]) !!}
                        </div>
                        <div class="checkbox">
                    ...
                    @foreach(Setting::get('locales') as $lang => $locale)
                        <div class="tab-pane fade @if ($loop->first)in active @endif"
                            ...
                            <div class="form-group">
                                ...
                                {!! Form::text( $lang . '[name]', (isset($customization)) ? $customization->{'name:'.$lang} : null,['class'=>'form-control','placeholder'=>__('backend.name').' '.$lang,'data-validation'=>'required', 'required']) !!}
                            </div>
                            <div class="form-group">
                                <label>@lang('backend.name_short')</label>
                                {!! Form::text( $lang . '[name_short]', (isset($customization)) ? $customization->{'name_short:'.$lang} : null,['class'=>'form-control','placeholder'=>__('backend.name_short').' '.$lang]) !!}
                            </div>
                            <div class="form-group">
                                <label>@lang('backend.description')</label>
                                {!! Form::textarea( $lang . '[description]', (isset($customization)) ? $customization->{'description:'.$lang} : null,['class'=>'form-control ck-editor','placeholder'=>__('backend.description').' '.$lang]) !!}
                            </div>
                        </div>
                    @endforeach
                </div>
...
@include('backend.elements.save_buttons', ['back_link' => route('backend.pages.customizations.index')])
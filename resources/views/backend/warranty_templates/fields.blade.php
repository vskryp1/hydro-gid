                    ...
                    @foreach(Setting::get('locales') as $lang => $locale)
                        <div class="tab-pane fade @if ($loop->first)in active @endif"
                            ...
                            <div class="form-group">
                                ...
                                {!! Form::text( $lang . '[name]', (isset($warranty_template)) ? $warranty_template->{'name:'.$lang} : null,['class'=>'form-control','placeholder'=>__('backend.name').' '.$lang,'data-validation'=>'required', 'required']) !!}
                            </div>
                            <div class="form-group">
                                <label>@lang('backend.template')</label>
                                {!! Form::textarea( $lang . '[template]', (isset($warranty_template)) ? $warranty_template->{'template:'.$lang} : null,['class'=>'form-control ck-editor','placeholder'=>__('backend.template').' '.$lang, 'data-validation'=>'required', 'required']) !!}
                            </div>
                        </div>
                    @endforeach
                        <a class="collapsed" data-toggle="collapse"
                           href="#collapseinfo"><i class="fa fa-info-circle"></i> @lang('backend.you_can_use_placeholders')</a>
                        <div id="collapseinfo" class="panel-collapse collapse">
                            <p>{product_warranties} - @lang('backend.product_warranties_placeholder')</p>
                        </div>
...
@include('backend.elements.save_buttons', ['back_link' => route('backend.products.warranty_templates.index')])
                    ...
                    @foreach(Setting::get('locales') as $lang => $locale)
                        <div class="tab-pane fade @if ($loop->first)in active @endif"
                            ...
                            <div class="form-group">
                                ...
                                {!! Form::text( $lang . '[name]', (isset($category)) ? $category->{'name:'.$lang} : null,['class'=>'form-control','placeholder'=>__('backend.name').' '.$lang,'data-validation'=>'required', 'required']) !!}
                            </div>
                        </div>
                    @endforeach
                </div>
...
@include('backend.elements.save_buttons', ['back_link' => route('backend.faqs.categories.index')])
<div class="js-basket__item basket__item">
    <a data-method="DELETE"
       class="js-basket-item-delete basket__item-delate"
       data-href="{{ route('ajax.cart.remove', $item->rowId) }}">
    ...
    <img class="basket__item-img" src="{{ $item->model->cover->getUrl('prod_cart') }}"
         alt="{{ $item->model->cover->getImageAlt() }}"
         title="{{ $item->model->cover->getImageTitle() }}">
    <div class="basket__item-info">
        <div class="basket__item-text">
            <a href="{{ $item->model->alias }}">
                <span class="name">{{ $item->model->name }} {{ $item->getWarrantyText() }} </span>
            </a>
        </div>
        <div class="basket__item-rating">
            <div class="star-box">
                <div data-mark="{{ round($item->model->product_reviews->average('rating')) }}"
                    ...
                    <input name="score" type="hidden"
                           value="{{ round($item->model->product_reviews->average('rating')) }}"
                           readonly>
                </div>
            </div>
        </div>
        <div class="basket__item-vendor">
            @lang('frontend/checkout/index.sku'):
            <span>{{ $item->model->sku }}</span>
            <i class="icon icon-copy js-copy"></i>
            <input class="input-copy" type="text" value="{{ $item->model->sku }}">
        </div>
    </div>
    <div class="basket__item-summ">
        <div class="quantity-cart basket__quantity-cart js_quantity">
            ...
            <input data-action="{{ route('ajax.cart.update', ['cart_id' => $item->rowId, 'count' => null]) }}"
                   type="text"
                   data-method="PUT"
                   class="js-changeAmount count"
                   value="{{ $item->qty }}"
                   readonly="">
            <span class="js_plus btn-count btn-count--plus">+</span>
        </div>
    </div>
    <div data-price="{{ $item->getUnitPriceWithWarranty() }}"
            data-discount-price="{{ $item->getUnitPriceWithWarranty(true) }}"
            class="js-price-block basket__item-prise">
        <strong data-sum-price="{{ $item->getSumPriceByPosition() }}"
                data-sum-discount-price="{{ $item->getSumPriceByPosition(true) }}"
                class="js-sum-item-price">
